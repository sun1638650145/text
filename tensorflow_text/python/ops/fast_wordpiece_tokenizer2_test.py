...
#       self.assertAllEqual(end, expected_end)
  ...
  def DISABLED_testWordPieceOpWithMultipleRaggedRank(
      self,
      tokens,
      expected_subwords,
      vocab,
  ...
  def DISABLED_testWordPieceOpWithIdReturned(self):
  ...
  def DISABLED_testWordPieceOpWithInt32IdReturned(self):
    """Let the table determine how to do a lookup on unknown tokens."""
    tokens = ragged_factory_ops.constant(
        [[b"don't", b"tread", b"cantfindme", b"treadcantfindme"]])
    tokenizer = FastWordpieceTokenizer(
...
class FastWordpieceOpAdditionalTest(test_base.DatasetTestBase,
  ...
  def DISABLED_testTokenizerBuiltInsideTfFunctionFromConfig(
  ...
  def DISABLED_testTokenizerBuiltOutsideTfFunctionFromConfig(
      self, text_inputs, expected_outputs):
    tokenizer = FastWordpieceTokenizer(
        vocab=_TEST_VOCAB,
        max_bytes_per_word=_TEST_MAX_BYTES_PER_WORD,
